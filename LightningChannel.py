...
def sign(message, message_types, signer_account):
    ...
    :param message: message to sign on (array).
    :param message_types: types of the message's components (array)
    :param signer_account: acocunt num. of the node who signs.
    :return: signature on the hashed message.
    """
...
def check_sig(message, message_types, sig, signer_pub_key):
    ...
    :param message: message to verify (array).
    :param message_types: types of the message's components (array)
    :param sig: signature on the hash of the given message.
...
def get_v_r_s(sig):
    """
    extract signature's components in the required form.
    :param sig: signature.
    :return: v, r, s (as in lecture)
    """
...
class LightningNode:
    """
    Defined the object of a node in Lightning network.
    """
    __last_seen_serial = 0  # The last serial number I've got / sent.
    __account = None
    __best_offer = None  # The highest amount of money we received.
    ...
    def __init__(self, my_account):
        ...
        :param my_account: the node's account number.
        """
    ...
    def establish_channel(self, other_party_address, amount_in_wei):
        ...
        :param other_party_address: address of the other participant in the channel.
        :param amount_in_wei: amount to deposit in the contract.
    ...
    def send(self, contract_address, amount_in_wei, other_node):
        """
        Sends money to the other address in the channel, and notifies the other node (calling its
        recieve()).
        :param contract_address: address of the contract.
        :param amount_in_wei: amount to send to the other party.
        :param other_node: other node's address.
        """
    ...
    def receive(self, pub_key, msg, signed_msg):
        """
        ...
        :param signed_msg: received signature.
        ...
        assert msg[CONTRACT_IDX] == self.__channel.address, "Unknown contract address! Maybe it's " \
                                                            "fake?"
        assert msg[RECEIVER_IDX] == self.__account, "The money is not delivered into the correct " \
                                                    "account."
        assert msg[AMOUNT_IDX] >= self.__best_offer, "We have a better offer already."
        assert msg[SERIAL_IDX] > self.__last_seen_serial, "Other party sends us an old serial!"

        self.__best_offer = msg[AMOUNT_IDX]
        self.__last_seen_serial = msg[SERIAL_IDX]

        # If everything is ok we can sign:
        self.__last_state = [msg, signed_msg]
        return sign(msg, self.__msg_types, self.__account)  # force the recipient to sign on the
        # message he received

    def unilateral_close_channel(self, contract_address, channel_state=None):
        """
        Closes the channel at the given contract address.
        :param contract_address: address of the channel which is to be closed.
        ...
        close_tx_hash = contract_address.functions.one_sided_close(msg[CONTRACT_IDX],
                                        msg[RECEIVER_IDX], msg[AMOUNT_IDX],
                                        msg[SERIAL_IDX], sig[V_IDX],
                                        sig[R_IDX], sig[S_IDX]).transact({"from": self.__account})
    ...
    def get_current_signed_channel_state(self, chan_contract_address):
        ...
        :param chan_contract_address: the address of the channel's contract.
    ...
    def appeal_closed_chan(self, contract_address):
        ...
        :param contract_address: contract's address.
        """
        ...
        if channel_current_serial < self.__last_state[MSG_IDX][SERIAL_IDX]:
            ...
            msg = self.__last_state[MSG_IDX]
            close_tx_hash = contract_address.functions.appeal_closure(msg[CONTRACT_IDX],
                                        msg[RECEIVER_IDX],
...
def wait_k_blocks(k: int, sleep_interval: int = 2):
    """
    insert a delay of k blocks.
    :param k:  length of delay (in blocks).
    """
